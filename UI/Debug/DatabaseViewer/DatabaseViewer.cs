...
public partial class DatabaseViewer : BaseDebugModule
{
    ...
    private void OnCategorySelected(long index)
    {
        ...
        {
            _dataTree.Clear();
            _detailPanel.Text = $"[color=yellow]No providers in category: {_selectedCategory}[/color]";
            return;
        }
    ...
    private void PopulateProviderList(List<IDataProvider> providers)
    {
        _dataTree.Clear();
        ...
        {
            var item = _dataTree.CreateItem(root);
            item.SetText(0, provider.Name);
            item.SetMetadata(0, Variant.From(provider.Name));
            item.SetText(1, $"[{provider.Category}]");
        }

        _detailPanel.Text = $"[color=cyan]{providers.Count}[/color] providers in [color=yellow]{_selectedCategory}[/color]\nClick a provider to view its data.";
    }
    ...
    private void OnTreeItemSelected()
    {
        ...
        {
            var providerName = metadata.AsString();
    ...
    private void UpdateDetailPanel()
    {
        ...
        var text = $"[color=cyan]Path:[/color] {path}\n";
        text += $"[color=cyan]Type:[/color] {type}\n";

        if (!string.IsNullOrEmpty(value))
        {
            text += $"[color=cyan]Value:[/color] {value}";
        }
    ...
    private void RefreshDataTree()
    {
        ...
        if (data == null)
        {
            _detailPanel.Text = "[color=red]Provider returned null data.[/color]";
            return;
        }
    ...
    private void OnRefreshPressed()
    {
        ...
        else if (!string.IsNullOrEmpty(_selectedCategory))
        {
            ...
            OnCategorySelected(_categoryList.GetSelectedItems()[0]);
        }
    }
    ...
    private void CopyPathToClipboard()
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            ...
            _detailPanel.Text = $"[color=green]Copied to clipboard:[/color] {path}";
        }
    }
    ...
    private void CopyValueToClipboard()
    {
        ...
        if (!string.IsNullOrEmpty(value))
        {
            ...
            _detailPanel.Text = $"[color=green]Copied value to clipboard[/color]";
        }
    }
    ...
    private void ToggleWatch()
    {
        ...
        if (_watchedPaths.Contains(path))
        {
            ...
            _detailPanel.Text = $"[color=yellow]Stopped watching:[/color] {path}";
        }
        else
        {
            _watchedPaths.Add(path);
            _watchButton.ButtonPressed = true;
            _detailPanel.Text = $"[color=green]Watching:[/color] {path}";
        }
    }
